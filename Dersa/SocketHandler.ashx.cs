...
namespace Dersa
{
    ...
    public class SocketHandler : IHttpHandler
    {
        ...
        private async Task WebSocketRequest(AspNetWebSocketContext context)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    //};
                    object scriptResult = MIinner.Item2.Invoke(MIinner.Item1, new object[] { communicationObject });
        ...
        private string ReceiveTextFromClient()
        {
            ...
            var buffer = new ArraySegment<byte>(new byte[1024]);
            // Ожидаем данные от него
            var readResult = socket.ReceiveAsync(buffer, CancellationToken.None);
            readResult.Wait();
            string result = Encoding.UTF8.GetString(buffer.ToArray<byte>(), 0, readResult.Result.Count);
            return result;
        }
